...
/**
 * Interpolate a value while preserving native JSON/YAML types.
 * Plain strings still interpolate as text; strings that are exactly one
 * variable/expression return that value's original type.
 */
function interpolateValue(value, context) {
  if (typeof value === 'string') {
    const variableOnly = value.match(/^\$([a-zA-Z_][a-zA-Z0-9_.]*)$/);
    if (variableOnly) return resolveVar(variableOnly[1], context);

    const expressionOnly = value.match(/^\{\{\s*([^}]+)\s*\}\}$/);
    if (expressionOnly) return evaluateExpr(expressionOnly[1].trim(), context);
